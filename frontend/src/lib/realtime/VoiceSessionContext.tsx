...
export function VoiceSessionProvider({
  ...
  // Remote LiveKit audio tracks are only *received* by the SDK —
  // nothing plays them until we attach() each subscribed track
  // to a real <audio> element.
  //
  // This container holds those elements (hidden; audio doesn't need
  // to be visible to play) so participants can actually hear each other.
  const audioContainerRef = useRef<HTMLDivElement | null>(null);

  const attachedElsRef = useRef<Map<string, HTMLMediaElement>>(
    new Map(),
  );
  ...
  // The public toggleMute/toggleDeafen need this to be defined above,
  // but joinPartyVoice also needs to apply the current output volume
  // to a freshly connected room's participants as they subscribe.
  useEffect(() => {
    ...
    const applyVolume = () => {
      room.remoteParticipants.forEach((p) =>
        p.setVolume(
          deafened ? 0 : outputVolumeState / 100,
        ),
      );
    };
  ...
  const toggleMute = useCallback(async () => {
    ...
    if (activeParty) {
      partyApi
        .setVoiceState(activeParty.id, { muted: next })
        .catch(() => {});
    }
  }, [muted, activeParty]);
  ...
  const toggleDeafen = useCallback(() => {
    ...
    // Deafening sets remote playback volume to 0 via LiveKit's
    // participant API rather than reaching into the DOM.
    //
    // Also force-mutes the mic, since if you can't hear the party
    // you shouldn't be broadcasting to it either.
    roomRef.current?.remoteParticipants.forEach((p) =>
      p.setVolume(
        next ? 0 : outputVolumeState / 100,
      ),
    );
  ...
  // Clean up the LiveKit connection on logout
  // so we don't leak a live mic.
  useEffect(() => {
    if (status !== 'authenticated') {
      teardownRoom();
    }
  ...
  return (
    <VoiceSessionContext.Provider value={value}>
      ...
      {/* Hidden mount point for remote participants' <audio> elements. */}
      <div
